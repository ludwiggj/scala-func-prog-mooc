...
object Anagrams {
  ...
  /** `Occurrences` is a `List` of pairs of characters and positive integers saying how often the character appears.
    * Characters lowercase, occurrence of character is 1 or greater
    * Sorted alphabetically w.r.t. to the character in each pair.
    */
  ...
  /** Converts a word into its character occurence list.
    * Uppercase and lowercase version of the character are treated as the same character
    */
...
//    dictionaryByOccurrences.get(wordOccurrences(word)) getOrElse (List())
  ...
  /** Subtracts occurrence list `y` from occurrence list `x`.
    *
    * The precondition is that the occurrence list `y` is a subset of the occurrence list `x
    * Any character appearing in `y` must appear in `x`, and its frequency in `y` must be
    * smaller or equal than its frequency in `x`.
    *
