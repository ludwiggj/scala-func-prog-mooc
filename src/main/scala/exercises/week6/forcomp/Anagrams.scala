...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap) {
      case (xMap, (k, v)) => {
        val remainingCount = xMap.apply(k) - v
  ...
  def subtract2(x: Occurrences, y: Occurrences): Occurrences = {
    y.toMap.foldLeft(x.toMap) {
      case (xMap, entry) => {
        val remainingCount = xMap.apply(entry._1) - entry._2
        if (remainingCount == 0) xMap - entry._1 else xMap.updated(entry._1, entry._2)
      }
    }.toList.sorted
  }

  def subtract3(x: Occurrences, y: Occurrences): Occurrences = {
    val yMap = y.toMap
    ...
    (x.toMap map (updateMapEntry(_))).toList.filter(_._2 > 0).sorted
  }
