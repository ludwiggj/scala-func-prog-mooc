package lectures.week4.objects_everywhere

abstract class Boolean {
  def ifThenElse[T](t: => T, e: => T): T // to be defined in subclass

  def && (x: => Boolean): Boolean = {
    println("&& this [" + this + "] x [" + x + "] >> ifThenElse(x, falsy)")
    ifThenElse(x, falsy)
  }

  def || (x: => Boolean): Boolean = {
    println("|| this [" + this + "] x [" + x + "] >> ifThenElse(truthy, x)")
    ifThenElse(truthy, x)
  }
...
object falsy extends Boolean {
  def ifThenElse[T](t: => T, e: => T) = {
    println("falsy.ifThenElse called for t [" + t + "] e [" + e + "] => e")
    e
  }
...
object truthy extends Boolean {
  def ifThenElse[T](t: => T, e: => T) = {
    println("truthy.ifThenElse called for t [" + t + "] e [" + e + "] => t")
    t
  }
