...
//  }
  ...
  test("sum of a few numbers including 0") {
    assert(sum(List(0, 12, 5, 0)) === 17)
  }
  
  test("sum of a few numbers including negative numbers") {
    assert(sum(List(0, -12, 5, 0, 16, -25)) === -16)
  }
  
  test("sum of a few numbers including repeated numbers") {
    assert(sum(List(4, -4, 4, -4, 4, -4)) === 0)
  }
  ...
  test("max of a few numbers including 0") {
    assert(max(List(0, 12, 5, 0)) === 12)
  }
  
  test("max of a few numbers including negative numbers") {
    assert(max(List(0, -12, 5, 0, 16, -25)) === 16)
  }
  
  test("max of a few numbers including repeated numbers") {
    assert(max(List(4, -4, 4, -4, 4, -4)) === 4)
  }
  
  test("max throws an exception for an empty list") {
    intercept[IllegalArgumentException] {
      max(List())
    }
  }
